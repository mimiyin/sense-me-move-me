...
function preload(){
  sound = loadSound("data/police.wav");
}
...
function draw() {
  ...
  switch(input) {
    ...
    // Acceleration
    case 2:
      value = abs(speed-pspeed)*width*0.1;
      break;
  }
  ...
  switch(mapping) {
    ...
    // Sine wave
    case 4:
        volume = (sin(2*TWO_PI*value/width) + 1);
        break;
    // Noise
    case 5:
        if(frameCount%2 == 0) {
  ...
  textSize(12);
  text("Press key to change input. Press mouse to change mapping.", 10, 20);
  textSize(18);
